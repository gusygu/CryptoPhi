...
const MATRIX_TABLE = String(ARG.get("table") ?? process.env.MATRIX_TABLE ?? "public.dyn_matrix_values").trim();
const STR_TABLE = String(ARG.get("table2") ?? "strategy_aux.str_aux_session").trim();
