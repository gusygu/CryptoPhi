...
const FEATURE_LINKS: NavLink[] = [
  ...
  { href: "/snapshot", label: "Snapshots" }, // 👈 NEW
] as const;


const DEV_LINKS: NavLink[] = [
  { href: "/admin", label: "Dashboard" },
  { href: "/admin/mgmt", label: "Manager Mgmt" },
  { href: "/admin/mail", label: "Comms" },
        ...
        <section className="rounded-xl border border-white/10 bg-zinc-950/70 p-4 text-[11px]">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              onClick={handleSnap}
              disabled={isSnapping}
              className="rounded-md border border-emerald-500/50 bg-emerald-600/30 px-2 py-1 text-[10px] font-medium text-emerald-50 transition hover:bg-emerald-600/45 disabled:opacity-60"
            >
          ...
          {!snapError && lastSnapAt && (
            <p className="mt-1 text-[10px] text-emerald-300">
              Snapshot saved {formatSince(lastSnapAt)}.
            </p>
          )}
