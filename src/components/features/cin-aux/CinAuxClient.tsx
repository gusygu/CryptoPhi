...
const CinAuxClient: React.FC<CinAuxClientProps> = ({
  ...
  return (
  <div className="flex flex-col gap-4 p-4 md:p-6 lg:p-8 w-full h-full text-slate-800 font-[var(--font-num)]">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
      {assets.map((a, idx) => {
        ...
        return (
          <div
            key={`${a.assetId}-${idx}`}
            className="border border-slate-200 rounded-lg p-3 bg-white flex flex-col justify-between"
          >
...
const MoveTable: React.FC<{ moves: CinRuntimeMoveRow[] }> = ({ moves }) => {
  ...
  return (
    <>
      <div className="flex flex-wrap gap-4 text-xs text-slate-700 mb-2">
        <span>
...
const AssetTauTable: React.FC<{ rows: CinAssetTauRow[] }> = ({ rows }) => {
  ...
  return (
    <div className="border rounded-lg bg-white/80 overflow-auto mt-3">
      <table className="min-w-full text-xs">
        <thead className="bg-gray-50 border-b text-slate-600">
          <tr>
        ...
        <tbody>
          {rows.map((row) => (
            ...
            >
              ...
              <td className="px-2 py-1 font-mono text-slate-700">
                {formatUsd(row.luggageUsdt)}
              </td>
            </tr>
          ))}
          <tr className="bg-gray-50 font-semibold text-slate-800">
