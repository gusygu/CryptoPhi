...
export function resolveBadgeFromCookies(jar: Awaited<ReturnType<typeof cookies>>): string {
  const legacy =
...
export async function saveUniverseAction(form: FormData): Promise<void> {
  ...
  if (session.isAdmin) {
    ...
    try {
      await withDbContext(
        { userId: session.userId, sessionId: badge, isAdmin: session.isAdmin, path: "/settings/saveUniverse" },
        async (client) => {
          const before = await client.query<{ cnt: number }>(
            `select count(*)::int as cnt from user_space.session_coin_universe where session_id = $1`,
            [badge],
          );
          const res = await upsertSessionCoinUniverse(
            badge,
            nextCoins,
            { enable: true, context: { userId: session.userId, isAdmin: session.isAdmin } },
            client,
          );
          const after = await client.query<{ cnt: number; sid: string | null; uid: string | null }>(
            `select count(*)::int as cnt,
                    nullif(current_setting('app.session_id', true), '') as sid,
                    nullif(current_setting('app.user_id', true), '')   as uid
