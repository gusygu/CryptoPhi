...
export function buildMeaAuxForCycle(params: {
  coins: string[];
  idPct: IdPctGrid;
  balances: BalancesMap;
  k?: number;
  ...
  const rows: MeaRow[] = [];
  for (const base of params.coins) {
    for (const quote of params.coins) {
      if (base === quote) continue;
      const id_pct = params.idPct?.[base]?.[quote] ?? null;
      const weight = grid?.[base]?.[quote] ?? null;
      rows.push({
        base,
        quote,
        id_pct,
        weight,
        tierName: id_pct != null ? pickTierName(id_pct, params.rules) : undefined,
        isNull: weight == null,
      });
    }
  }
...
export function toRenderableRows(mea: Record<string, Record<string, {
  ...
  const rows: MeaRow[] = [];
  for (const [base, cols] of Object.entries(mea)) {
    for (const [quote, info] of Object.entries(cols)) {
      rows.push({ base, quote, ...info });
    }
  }
  return rows;
}
