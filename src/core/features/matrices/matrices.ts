...
export async function composeMatrices(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const pctSnap = out.pct_snap[i][quoteIdx];
    const snapVal = out.snap[i][quoteIdx];
    ...
    const pctSnapCell: Cell = { value: pctSnap, color: asPctColor(pctSnap, frozen),      derivation: deriv, ring: cellRing };
    const snapCell:     Cell = { value: snapVal, color: asPctColor(pctSnap, frozen),     derivation: deriv, ring: cellRing };
    ...
    rows.push({
      ...
      snap_block:       { top: pctSnapCell, bottom: snapCell },
      delta:   deltaCell,
      id_pct:  idPctCell,
      pct_drv: drvCell,
      meta: { frozen }
