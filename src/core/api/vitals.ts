...
const ORDERBOOK_DEPTHS = [5, 10, 20, 50, 100, 500, 1000] as const;
type OrderbookDepth = (typeof ORDERBOOK_DEPTHS)[number];

function resolveDepth(raw?: number): OrderbookDepth {
  const fallback: OrderbookDepth = 20;
  if (!Number.isFinite(raw)) return fallback;
  const value = Number(raw);
  const exact = ORDERBOOK_DEPTHS.find((candidate) => candidate === value);
  if (exact) return exact;
  let nearest: OrderbookDepth = fallback;
  let minDelta = Number.POSITIVE_INFINITY;
  for (const candidate of ORDERBOOK_DEPTHS) {
