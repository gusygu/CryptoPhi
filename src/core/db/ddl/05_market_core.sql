...
-- Ensure view creation doesn't fail if ops.session_flags doesn't exist yet.
DO $ddl$
BEGIN
  IF to_regclass('ops.session_flags') IS NULL THEN
    RAISE NOTICE 'ops.session_flags missing; skipping market.v_session_open';
    RETURN;
  END IF;
