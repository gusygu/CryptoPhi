...
DO $mat_health$
BEGIN
  IF to_regclass('cin_aux.mat_registry') IS NOT NULL
     AND to_regclass('cin_aux.mat_cell') IS NOT NULL THEN
    EXECUTE $$
  ...
  ELSE
    EXECUTE $$
      CREATE OR REPLACE VIEW vitals.matrices_health AS
      SELECT
        0::bigint AS mat_registry_rows,
        0::bigint AS mat_cell_rows,
        (to_regclass('matrices.dyn_values') IS NOT NULL) AS dyn_values_exists
    $$;
  END IF;
END
...
DO $dyn_health$
BEGIN
  IF to_regclass('mea_dynamics.mea_symbol') IS NOT NULL
     AND to_regclass('mea_dynamics.dynamics_snapshot') IS NOT NULL
  ...
  ELSE
    EXECUTE $$
      CREATE OR REPLACE VIEW vitals.dynamics_health AS
      SELECT 0::bigint AS mea_symbol_rows,
             0::bigint AS snapshot_rows,
             0::bigint AS latest_rows
    $$;
  END IF;
END
$dyn_health$;
...
-- 28_admin_internal_id.sql
BEGIN;

-- Add a manually-maintained internal admin ID to managers.
-- This can be used in admin client ledgers, UI badges, etc.
-- It is NOT the primary key, just a human-readable code.

DO $patch$
BEGIN
  IF to_regclass('admin.managers') IS NULL THEN
    RAISE NOTICE 'admin.managers not found; skipping internal_admin_id patch';
    RETURN;
  END IF;

  EXECUTE '
    ALTER TABLE admin.managers
      ADD COLUMN IF NOT EXISTS internal_admin_id text
  ';

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'admin.managers'::regclass
      AND conname = 'chk_managers_internal_admin_id_format'
  ) THEN
    EXECUTE $$
      ALTER TABLE admin.managers
        ADD CONSTRAINT chk_managers_internal_admin_id_format
        CHECK (
          internal_admin_id IS NULL
          OR internal_admin_id ~ '^(ALPHA|BETA|GAMMA|DELTA|EPSILON|ZETA|ETA|THETA|IOTA|KAPPA|LAMBDA|MU|NU|XI|OMICRON|PI|RHO|SIGMA|TAU|UPSILON|PHI|CHI|PSI|OMEGA)-(0[0-9]{2}|100)[A-Za-z](100|[1-9]?[0-9])$'
        )
    $$;
  END IF;

  EXECUTE '
    CREATE UNIQUE INDEX IF NOT EXISTS uq_managers_internal_admin_id
      ON admin.managers (internal_admin_id)
      WHERE internal_admin_id IS NOT NULL
  ';

  EXECUTE $$
    COMMENT ON COLUMN admin.managers.internal_admin_id IS
      'Human-readable internal admin/manager ID; greekCode-NNN<roman><0-100>, e.g. PI-007A0. Maintained manually by admins.'
  $$;
END
$patch$;
...
-- optional view in admin schema
DO $view$
BEGIN
  IF to_regclass('admin.action_log') IS NULL
     OR to_regclass('admin.managers') IS NULL THEN
    RAISE NOTICE 'admin.action_log or admin.managers missing; skipping admin.v_action_log_enriched';
    RETURN;
  END IF;

  EXECUTE $$
    CREATE OR REPLACE VIEW admin.v_action_log_enriched AS
    SELECT
      l.action_id,
      l.actor_role,
      l.actor_email,
      ...
      l.created_at,
      m.internal_admin_id
    FROM admin.action_log l
    LEFT JOIN admin.managers m
      ON m.manager_id = l.actor_manager_id
  $$;
END
$view$;

-- 15_admin_invites_user_generated.sql
-- Assumes schemas: admin, auth and table admin.invites already exist.

DO $invites$
BEGIN
  IF to_regclass('admin.invites') IS NULL THEN
    RAISE NOTICE 'admin.invites missing; skipping invite metadata patch';
    RETURN;
  END IF;

  IF to_regclass('auth.users') IS NULL THEN
    RAISE NOTICE 'auth.users missing; skipping invite metadata patch';
    RETURN;
  END IF;
  ...
  EXECUTE '
    CREATE UNIQUE INDEX IF NOT EXISTS ux_invites_single_user_invite
      ON admin.invites(inviter_user_id)
      WHERE source = ''user''
  ';
END
$invites$;
