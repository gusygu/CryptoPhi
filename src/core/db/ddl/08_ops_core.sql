...
-- enums now centralized in 03_core_types_enums.sql; keep guards for idempotency
DO $$
BEGIN
...
CREATE TABLE IF NOT EXISTS ops."order" (
  ...
  session_id uuid NOT NULL, -- FK attached later when cin_aux.sessions exists
...
BEGIN
  ...
  IF sess_exists THEN
    ...
    IF id_type = 'uuid' THEN
      -- ops.order.session_id main FK (only when cin_aux.sessions exists)
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_ops_order_session'
          AND conrelid = 'ops."order"'::regclass
      ) THEN
        ALTER TABLE ops."order"
          ADD CONSTRAINT fk_ops_order_session
