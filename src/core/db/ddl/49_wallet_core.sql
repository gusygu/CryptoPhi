...
DROP VIEW IF EXISTS cin_aux.v_rt_session_summary;
...
WHERE s.owner_user_id IS NULL OR s.owner_user_id = auth.current_user_id();

------------------------------------------------------------
-- 2) Per-asset PnL + last mark
------------------------------------------------------------

DROP VIEW IF EXISTS cin_aux.v_rt_session_recon;
DROP VIEW IF EXISTS cin_aux.v_rt_asset_pnl CASCADE;
...
WHERE b.owner_user_id IS NULL OR b.owner_user_id = auth.current_user_id();
...
WITH asset_pnl AS (
  ...
  GROUP BY session_id, owner_user_id
),
ref_total AS (
  SELECT
    session_id,
...
DROP VIEW IF EXISTS cin_aux.v_rt_asset_tau;
...
WHERE sl.owner_user_id IS NULL OR sl.owner_user_id = auth.current_user_id();

DROP VIEW IF EXISTS cin_aux.v_mea_alignment_scored;
