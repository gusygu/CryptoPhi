...
-- 16_auth_users_invited_by_user.sql

DO $auth_patch$
BEGIN
  IF to_regclass('auth.users') IS NULL THEN
    RAISE NOTICE 'auth.users missing; skipping invited_by_user_id patch';
    RETURN;
  END IF;
  ...
  EXECUTE '
    CREATE INDEX IF NOT EXISTS ix_users_invited_by_user
      ON auth.users(invited_by_user_id)
  ';
END
$auth_patch$;
