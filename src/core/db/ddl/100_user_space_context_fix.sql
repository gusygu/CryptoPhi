...
DO $BLOCK$
BEGIN
  -- Skip gracefully if user_space tables are not present yet (e.g., when this file runs before 40_user_space.sql).
  IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'user_space') THEN
    RAISE NOTICE 'Skipping user_space context fix: schema user_space is missing';
    RETURN;
  END IF;
  IF to_regclass('user_space.session_coin_universe') IS NULL THEN
    RAISE NOTICE 'Skipping user_space context fix: table user_space.session_coin_universe is missing';
    RETURN;
  END IF;
