...
begin
  begin
    execute 'drop function if exists public._split_symbol(text)';
  exception
    when undefined_function then null;
    when others then null;
  end;
  begin
    execute 'drop function if exists market._split_symbol(text)';
  exception
    when undefined_function then null;
    when others then null;
  end;
...
create schema if not exists str_aux; -- needed for optional stubs below
...
-- Guarded stale-target helper: returns empty set if str_aux.samples_5s is absent.
...
as $$
begin
  if to_regclass('str_aux.samples_5s') is null then
    return;
  end if;
...
-- Stub: ensure str_aux.upsert_sample_5s exists even before str_aux core DDL runs.
do $do$
begin
  if not exists (
    select 1
    ...
    where n.nspname = 'str_aux' and p.proname = 'upsert_sample_5s'
  ) then
    execute $fn$
      create or replace function str_aux.upsert_sample_5s(
      ...
      begin
        -- no-op stub; real implementation arrives with str_aux core DDL
        return;
      end;
      $$;
    $fn$;
