...
type SqlTransactionTag = <T extends QueryResultRow = QueryResultRow>(
...
async function runSqlTag<T extends QueryResultRow>(
  strings: TemplateStringsArray,
  values: any[]
): Promise<T[]> {
  ...
  },
  {
    begin: async <T>(fn: (tx: SqlTransactionTag) => Promise<T>): Promise<T> => {
      return withClient(async (client) => {
        ...
        try {
          const tx: SqlTransactionTag = async <R extends QueryResultRow = QueryResultRow>(
            strings: TemplateStringsArray,
            ...values: any[]
          ): Promise<R[]> => {
            const { text, params } = compileSql(strings, values);
