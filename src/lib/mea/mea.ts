...
import type { CoinId, MeaConfig, MeaResult, TierBand } from "./types";
...
type TierMetaRow = MeaResult["meta"]["tiers"][CoinId];
type MatrixMode = "vector" | "diagonal" | "outer";
...
export function computeMEA(params: {
  ...
  const bands = config?.tiers?.bands ?? DEFAULT_BANDS;
  const eps = config?.tiers?.eps ?? 1e-9;
  const clampMin = config?.clampMinAllocation ?? 0;
  const clampMax = config?.clampMaxAllocation ?? Infinity;
  ...
  for (const base of coins) {
    ...
    for (const quote of coins) {
      ...
      tiersMeta[base][quote] = { band: tier, weight, z };
      agg += weight;
    }
  ...
  return {
    weights,
    matrix,
    meta: { moodCoeff, tiers: tiersMeta, params: { nCoins: coins.length } },
  };
}
