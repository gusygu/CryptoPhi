...
export default function Matrix({
  ...
  const denseMode = rows >= DENSE_THRESHOLD;
  const tableClass = [
    "min-w-full border-separate border-spacing-x-0",
    denseMode ? "border-spacing-y-0.5 text-[9px]" : "border-spacing-y-1 text-[10px]",
  ].join(" ");
  const columnHeaderClass = denseMode
    ? "px-1.5 py-1 text-right font-mono uppercase tracking-[0.14em] text-slate-400"
    : "px-2 py-1.5 text-right font-mono uppercase tracking-[0.18em] text-slate-400";
  const rowHeaderClass = denseMode
    ? "px-1.5 py-1 text-left text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-300"
    : "px-2.5 py-1.5 text-left font-semibold uppercase tracking-[0.18em] text-slate-300";
  const cellPaddingClass = denseMode ? "px-1 py-0.5" : "px-1 py-1";
  const innerCellPadding = denseMode ? "px-1.5 py-1" : "px-2 py-1.5";
  const detailTextClass = denseMode ? "text-[9px]" : "text-[10px]";
            ...
            <thead>
              <tr className="text-slate-400">
                ...
                {Array.from({ length: cols }).map((_, j) => (
                  <th key={`head-${j}`} className={columnHeaderClass}>
                    {coins[j] ?? "?"}
                  </th>
                ))}
              </tr>
              ...
              {Array.from({ length: rows }).map((_, i) => (
                <tr key={`row-${coins[i] ?? i}`}>
                  ...
                  {Array.from({ length: cols }).map((__, j) => {
                    ...
                    return (
                      <td key={`cell-${i}-${j}`} className={cellPaddingClass}>
                        <div
                          className={`relative rounded-lg ${innerCellPadding} text-right font-mono tabular-nums ${textClass} ${
                        ...
                        >
                          ...
                          {hasDetail ? (
                            <span
                              className={`block ${detailTextClass} leading-tight`}
